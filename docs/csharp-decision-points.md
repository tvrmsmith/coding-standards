...
| a switch expression arm | not counted. `hasConditionalLogic` has no case for a switch expression arm or for any pattern operation kind | +1 per arm, since 2026-09-03 | a twenty-arm switch expression scored complexity 1 under the walker before that date, while the same logic written as a `switch` statement scored 20; scoring each arm closed that blind spot |
| a pattern combinator (`and`, `or`) | not counted. A pattern combinator lowers to a pattern operation, never to `OperationKind.BinaryOperator`, so it never reaches the operator-kind check `hasConditionalLogic` runs | +1 per combinator, since the same date | a recursive pattern's `and`/`or` does the same job `&&`/`||` does in a boolean expression, which the extractor already scores |
...
pipe as the gate's delimiter, and the comma lost because it would force `Order.Map<TKey, TValue>` to
be quoted where pipe never needs to quote it.
