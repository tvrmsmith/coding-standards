...
func indexGit(t *testing.T, dir, index string, args ...string) string {
	t.Helper()
	//nolint:gosec // the literal "git" with argv each case writes out, run in that case's own t.TempDir repository with the machine's git config pointed at os.DevNull two lines down
